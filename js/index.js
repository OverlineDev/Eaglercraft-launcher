...
function generateprofile(game) {
    if (!selectedGame1) {
        fetch("./assets/json/base.json")
            .then((response) => response.json())
            .then((data) => {
                selectedGame1 = JSON.stringify(data[0]);
                localStorage.setItem("basegame", selectedGame1);
    ...
    if (!selectedGame2) {
        fetch("./assets/json/modded.json")
            .then((response) => response.json())
            .then((data) => {
                selectedGame2 = JSON.stringify(data[0]);
                localStorage.setItem("moddedgame", selectedGame2);
    ...
    if (!selectedGame3) {
        fetch("./assets/json/assisted.json")
            .then((response) => response.json())
            .then((data) => {
                selectedGame3 = JSON.stringify(data[0]);
                localStorage.setItem("assisted", selectedGame3);
...
function updateGameDisplay(gameJson) {
    if (!gameJson) return;
    const selectedGame = JSON.parse(gameJson);
...
function generategames(path) {
    ...
    fetch(path)
        ...
        .then((data) => {
            data.reverse();
            data.forEach((game) => {
                if (!localStorage.getItem(game.title) && game.active === false) {
                    localStorage.setItem(game.title, game.active);
                    return;
                }
                if (localStorage.getItem(game.title) === 'false') {
                    return;
                }
                ...
                gameoption.addEventListener("click", () => {
                    ...
                    if (path === "./assets/json/base.json") {
                        selectedGame1 = JSON.stringify(game);
                        localStorage.setItem("basegame", selectedGame1);
                    }
                    if (path === "./assets/json/modded.json") {
                        selectedGame2 = JSON.stringify(game);
                        localStorage.setItem("moddedgame", selectedGame2);
                    }
                    if (path === "./assets/json/assisted.json") {
                        selectedGame3 = JSON.stringify(game);
                        localStorage.setItem("assisted", selectedGame3);
                    }
...
function dropdowntoggle() {
    if (dropdown.style.visibility === 'hidden') {
        ...
        document.getElementById('dropdownuparrow').innerHTML =
            '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="dropdownIcon"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M6 9l6 6l6 -6" /></svg>';
    } else {
...
function resetTabSelected() {
    ...
    // index.html has gtabs1..gtabs4 and header1..header4
    for (let i = 1; i <= 4; i++) {
        const gtabs = document.getElementById('gtabs' + i);
        const headers = document.getElementById('header' + i);
...
// Define openZIframe globally (not inside click handler)
...
if (userchosen === false && !localStorage.getItem("username")) {
    fetch("https://genr8rs.com/api/Content/Fun/XboxNameGenerator?genr8rsUserId=1748114452.233968321c14391c2&_sGameGenre=any")
        .then((response) => response.json())
        .then((data) => {
            if (!data) return;
...
document.addEventListener("click", function (event) {
    const dropdownEl = document.getElementById("dropdn");
